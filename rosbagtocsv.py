...
if (len(sys.argv) > 2):
	...
	print("should be 2: 'rosbagParser.py' and 'bagName'")
	print("or just 1  : 'rosbagParser.py'")
	sys.exit(1)
elif (len(sys.argv) == 2):
...
for bagFile in listOfBagFiles:
	...
	for topicName in listOfTopics:
		...
		with open(filename, 'w+') as csvfile:
			...
			print("Generating CSV File For The Topic " + topicName + " ...")
				...
				for nameValuePair in msgList:
					...
					if len(splitPair) == 2:
						for i in range(len(splitPair)):
							splitPair[i] = splitPair[i].strip(',').replace(" ", "")
	...
	bag.close()
print("Done CSV Convertion For All " + str(numberOfFiles) + " Bag Files.")